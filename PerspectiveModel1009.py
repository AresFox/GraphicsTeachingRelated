...
class PerspectiveModel1009(Scene):
    ...
    def construct(self):
        ...
        dotpointX = Dot(axes.c2p(pointX[0], pointX[1], pointX[2]), fill_color=PINK)
        pointXText = Text("X", font_size=40, color=colorAxis)
        pointXText.rotate(PI/2, axis=RIGHT)
        pointXText.next_to(dotpointX, RIGHT)
        self.add(pointXText)


        pointY = Dot(axes.c2p(pointY[0], pointY[1], pointY[2]), fill_color=PINK)
        pointYText = Text("-Z", font_size=40, color=colorAxis)
        pointYText.rotate(PI/2, axis=RIGHT)
        pointYText.next_to(pointY, RIGHT)
        self.add(pointYText)


        pointZ = Dot(axes.c2p(pointZ[0], pointZ[1], pointZ[2]), fill_color=PINK)
        pointZText = Text("Y", font_size=40, color=colorAxis)
        pointZText.rotate(PI/2, axis=RIGHT)
        ...
        base.set_fill(opacity=0.2, color=YELLOW)
        ...
        for i in range(4):
            ...
            side.set_fill(opacity=0.1, color=BLUE)
            sides.append(side)
        ...
        # 绘制矩形棱台观察体
        # 矩形棱台的八个顶点分别是base和far_plane的四个顶点
        # 绘制矩形棱台的八条边
        # 绘制多面体：
        # polyhedron = Polygon(
        #     axes.c2p(base_points[0][0], base_points[0][1], base_points[0][2]),
        #     axes.c2p(base_points[1][0], base_points[1][1], base_points[1][2]),
        #     axes.c2p(base_points[2][0], base_points[2][1], base_points[2][2]),
        #     axes.c2p(base_points[3][0], base_points[3][1], base_points[3][2]),
        #     axes.c2p(far_plane_points[3][0], far_plane_points[3][1], far_plane_points[3][2]),
        #     axes.c2p(far_plane_points[2][0], far_plane_points[2][1], far_plane_points[2][2]),
        #     axes.c2p(far_plane_points[1][0], far_plane_points[1][1], far_plane_points[1][2]),
        #     axes.c2p(far_plane_points[0][0], far_plane_points[0][1], far_plane_points[0][2]),
        #
        #
        #
        # )
        # polyhedron.set_fill(opacity=0.5, color=RED)
        # self.add(polyhedron)

            # 使用 axes.c2p 将点转换为坐标系中的点
        base_polygon = Polygon(*[axes.c2p(*point) for point in base_points], fill_color=BLUE, fill_opacity=0.5)
        top_polygon = Polygon(*[axes.c2p(*point) for point in far_plane_points], fill_color=GREEN_A, fill_opacity=0.5)

            # 添加坐标轴、底面和顶面
            # self.add(axes)
        self.add(base_polygon, top_polygon)

            # 绘制侧面
        side_faces = []
        for i in range(len(base_points)):
            p1 = axes.c2p(*base_points[i])
            p2 = axes.c2p(*base_points[(i + 1) % len(base_points)])
            p3 = axes.c2p(*far_plane_points[(i + 1) % len(far_plane_points)])
            p4 = axes.c2p(*far_plane_points[i])
            side_face = Polygon(p1, p2, p3, p4, fill_color=GREEN, fill_opacity=0.5)
